...
describe('DictationPreloadComponent', () => {
  ...
  it('should start with idle state for all categories', () => {
    ...
    expect(component.interpretations.state).toBe('idle');
  });

  it('start sets total for each category', () => {
    component.start({ questions: 5, voices: 5, images: 3, interpretations: 4 });
    expect(component.questions.total).toBe(5);
    expect(component.voices.total).toBe(5);
    expect(component.images.total).toBe(3);
    expect(component.interpretations.total).toBe(4);
  });

  it('completeCategory jumps loaded to total and marks done', () => {
    component.start({ questions: 5, voices: 5, images: 5, interpretations: 5 });
  ...
  it('recordVoice increments loaded on success', () => {
    component.start({ questions: 1, voices: 2, images: 1, interpretations: 0 });
  ...
  it('recordInterpretation increments loaded and reaches done', () => {
    component.start({ questions: 0, voices: 0, images: 0, interpretations: 2 });
    component.recordInterpretation(true);
    expect(component.interpretations.loaded).toBe(1);
    expect(component.interpretations.state).toBe('loading');
    component.recordInterpretation(true);
    expect(component.interpretations.state).toBe('done');
  });

  it('interpretations category with total 0 auto-completes', () => {
    component.start({ questions: 1, voices: 0, images: 0, interpretations: 0 });
    expect(component.interpretations.state).toBe('done');
  });
  ...
  it('emits preloadCompleted with useLocalVoice=false when all categories succeed', fakeAsync(() => {
    ...
    component.start({ questions: 1, voices: 1, images: 1, interpretations: 1 });
    component.completeCategory(PreloadCategoryName.Questions);
    component.completeCategory(PreloadCategoryName.Images);
    component.completeCategory(PreloadCategoryName.Interpretations);
    component.recordVoice(true);
  ...
  it('shows continue button when voices fail', fakeAsync(() => {
    component.start({ questions: 1, voices: 1, images: 1, interpretations: 0 });
    component.completeCategory(PreloadCategoryName.Questions);
    component.completeCategory(PreloadCategoryName.Images);
    component.recordVoice(false);
    tick(500);
